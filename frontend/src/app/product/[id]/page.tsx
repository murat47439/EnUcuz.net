...
export default async function ProductDetailPage({params} : {params : {id : number}}) {
    ...
    return (
        <main className="container mx-auto p-4 md:p-8">
            <h1 className="text-3xl font-bold text-center mb-16">{product.Product.name}</h1>
            <h4 className="text-blue-400"><u>
                <Link href={{pathname: "/",
                query: { category: product.Product.category_id },
                }}>{product.Product.category_name}</Link>
                </u>
                 - <u><Link href={{pathname: "/",
                    query: { brand: product.Product.brand_id },}}>{product.Product.brand_name}</Link></u></h4>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] shadow-md p-4">
                <div>
                    ...
                    <p className="px-10 py-4">{product.Product.description}</p>
                </div>
